	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				changeDLGContent("申し訳ございません。注文【"+obj.attr("billcc")+"】削除失敗しました。");
			}else if(data.delResult == "notexist"){
				//alert("对不起，订单【"+obj.attr("billcc")+"】不存在");
				changeDLGContent("注文【"+obj.attr("billcc")+"】は存在しません。");
			}
		},
		error:function(data){
			alert("申し訳ございません。削除失敗しました。");
		}
	});
}
...
$(function(){
	...
	$(".deleteBill").on("click",function(){
		billObj = $(this);
		changeDLGContent("注文【"+billObj.attr("billcc")+"】を削除しますか？");
		openYesOrNoDLG();
	});
